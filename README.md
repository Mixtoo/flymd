...
| 扩展 | 介绍 |
|------|------|
| PDF 高精度解析 | 高精度解析 PDF 文本、表格与目录，适合技术文档和论文整理 |
| Git 版本管理 | 为当前库和当前文档提供 Git 侧栏，一键初始化本地仓库、查看提交历史与差异高亮，仅支持显式快照提交，不做强制重置等破坏性操作 |
| Markdown 表格助手 | 一键生成、对齐和重排 Markdown 表格，告别手工对齐的痛苦 |
| 协同编辑 | 通过协同服务多人实时编辑同一文档，支持光标同步和基础冲突处理 |
| Typecho 博文管理 | 在本地统一管理 Typecho 草稿、快照和回滚，支持一键发布与更新 |
