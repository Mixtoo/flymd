...
const dict = {
  zh: {
    ...
    'sync.allowHttp': '允许 HTTP 明文传输',
    'sync.allowHttp.warn': '⚠️ 打开后将不再强制使用 HTTPS，所有同步数据都会以明文发送，仅在完全可信的本地/局域网环境下勾选。',
    ...
    'sync.allowHttp': 'Allow HTTP (plaintext)',
    'sync.allowHttp.warn': '⚠️ Disables the HTTPS-only guard. Data will be sent in plaintext; enable only on trusted local/LAN servers.',
