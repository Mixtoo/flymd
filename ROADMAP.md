...
## 即将更新
- 所见模式与扩展能力全面对齐，AI 助手、待办等插件在所见模式中也能即时读取/写入内容，无需再切回源码模式。
- 右键菜单新增内置快捷项：图床一键开启/关闭、立即发起 WebDAV 同步、打开同步记录，并保留 `Shift + 右键` 的原生菜单提示。
- WebDAV 同步日志改用本地时间戳记录，并在文件超过 5MB 时自动截断，防止同步记录无限膨胀。
